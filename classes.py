...
class Ghost(Person):
    ...
    def pos(self, G):
        pygame.draw.rect(G.screen, G.RED,
                         [(G.margin + G.width) * self.x + G.margin, (G.margin + G.height) * self.y + G.margin, G.width,
                          G.height])
    def checkHero(self, H ,W):
        move = 0
        if self.x - H.x >  self.y - H.y:
            posY = self.y
            if posY == self.ghostdown(W).y:
                self.ghostright(W)
        elif self.x - H.x < self.y - H.y:
            posY = self.y
            if posY == self.ghostup(W).y:
                self.ghostleft(W)
        elif self.x - H.x > 0 :
            posX = self.x
            if posX == self.ghostright(W).x:
                self.ghostup(W)
        else:
            posX = self.x
            if posX == self.ghostleft(W).x:
                self.ghostdown(W)

    def ghostdown(self, W):
        move = randint(0,6)
        if move == 0 :
            self.ghleft(W)
        elif move == 1:
